...
function M.lsp()
    ...
    keymap(
        n,
        "ge",
        function() require("msotgia.native.lsp").open_diagnostics_float() end,
        default_settings
    )
    keymap(n, "[e", function() require("msotgia.native.lsp").prev_diag() end, default_settings)
    keymap(n, "]e", function() require("msotgia.native.lsp").next_diag() end, default_settings)
    keymap(n, "[E", function() require("msotgia.native.lsp").prev_error() end, default_settings)
    keymap(n, "]E", function() require("msotgia.native.lsp").next_error() end, default_settings)
...
function M.native()
    ...
    keymap(
        n,
        "<leader>d",
        function() require("msotgia.native.lsp").toggle_virtual_diagnostics() end,
        default_settings
    )
    keymap(
        n,
        "<leader>f",
        function() require("msotgia.native.lsp").toggle_format_enabled() end,
        default_settings
    )
    keymap(n, "gm", "<Cmd>vertical Man<CR>", default_settings)

    -- Buffers
    keymap(n, "Q", function() require("msotgia.keymaps.utils").delete_buffer() end, default_settings)
...
function M.oil()
    keymap(n, "<leader>e", function() require("msotgia.keymaps.utils").toggle_oil() end)
    require("oil").setup({
        keymaps = {
            ["<CR>"] = "actions.select",
            ["-"] = "actions.parent",
...
function M.editing()
    ...
    keymap(
        ex_t,
        "<C-s>",
        function() require("msotgia.keymaps.utils").save_file() end,
        default_settings
    )
    ...
    keymap(n, "<leader>v", function() require("msotgia.keymaps.utils").toggle_diffview() end)
end
