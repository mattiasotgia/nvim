...
if U.is_default() then
    local C = require("msotgia.native.themes.default").palette
    red = C.red
    green = C.green
    icon_hl = { fg = C.gray2 }
    text_hl = { fg = C.gray2 }
...
local function get_virtual_text_color()
    local enabled = require("msotgia.native.lsp").virtual_diagnostics
    if enabled then return { fg = green } end
    return icon_hl
end

local function get_format_enabled_color()
    local enabled = require("msotgia.native.lsp").format_enabled
    if enabled then return { fg = green } end
    return icon_hl
end
...
if U.is_default() then require("msotgia.native.themes.default").setup_lualine() end
